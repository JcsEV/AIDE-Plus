...
public class AIDEEditor extends com.aide.ui.AIDEEditor {
		...
		public void initColors() {
			...
			if (AndroidHelper.isMaterialTheme(getContext())) {
				...
				this.selectionColor = new Color(
						io.github.zeroaicy.aide.highlight.ColorKind.EDITOR_SELECTION.getColor(getContext(), isLight));
			} else {
				this.selectionColor = new Color(
						resources.getColor(isLight ? R.color.editor_selection_light : R.color.editor_selection, theme));
			}

			// 背景颜色
			//this.graphicsColor = new Color(getResources().getColor(isLight ? R.color.editor_background_light : R.color.editor_background));
			this.graphicsColor = new Color(
					io.github.zeroaicy.aide.highlight.ColorKind.EDITOR_BACKGROUND.getColor(context, isLight));

			// 光标所在行背景色
			this.Za = this.jn
					? new Color(resources
							.getColor(isLight ? R.color.editor_caret_line_light : R.color.editor_caret_line, theme))
					: null;

			this.stepbarColor = this.jn
					? new Color(resources
							.getColor(isLight ? R.color.editor_stepping_bar_light : R.color.editor_stepping_bar, theme))
					: null;

			this.Pa = new Color(resources.getColor(isLight ? R.color.editor_caret_light : R.color.editor_caret, theme));

			this.separatorColor = new Color(
					resources.getColor(isLight ? R.color.editor_separator_light : R.color.editor_separator, theme));
			this.hyperlinkColor = new Color(
					resources.getColor(isLight ? R.color.editor_hyperlink_light : R.color.editor_hyperlink, theme));

			this.Bx = new Color(resources
					.getColor(isLight ? R.color.editor_diff_inserted_light : R.color.editor_diff_inserted, theme));
			this.Jm = new Color(resources
					.getColor(isLight ? R.color.editor_diff_deleted_light : R.color.editor_diff_deleted, theme));
			this.An = new Color(
					resources.getColor(isLight ? R.color.editor_line_number_light : R.color.editor_line_number, theme));
	...
	protected OpenFileService.OpenFileModel Z1(final String filePath) {
		AIDEEditorModel editorModel;
		if (ZeroAicySetting.isEnableAsynRead()) {
			// 先返回，内容异步塞入
			editorModel = new AIDEEditorModel();
			editorModel.initAsyn(filePath);
		} else {
			editorModel = new AIDEEditorModel(filePath);
		}
		return editorModel;
	}
	...
	public class AIDEEditorModel extends com.aide.ui.AIDEEditor.t {
		...
		public void initAsyn(String filePath) {
			if (!this.isAsyn) {
				return;
			}
		...
		private void initAsync(Reader reader) {
			try {
				// 锁住自己
				synchronized (this) {
					// 读取
