...
public class SimpleHighlighterASTVisitor extends ASTVisitor {
	...
	public TextRange getTextRange(int sourceStart, int sourceEnd) {
		...
		int startLine = sourceStart >= 0 ? Util.getLineNumber(sourceStart, lineEnds, 0, lineEnds.length - 1) : 0;
		int startColumn = sourceStart >= 0 ? Util.searchColumnNumber(lineEnds, startLine, sourceStart) : 0;

		int endLine = sourceEnd >= 0 ? Util.getLineNumber(sourceEnd, lineEnds, 0, lineEnds.length - 1) : 0;

		int endColumn = sourceEnd >= 0 ? Util.searchColumnNumber(lineEnds, endLine, sourceEnd) + 1 : 0;

		SimpleHighlighterASTVisitor.TextRange textRange = new TextRange(startLine, startColumn, endLine, endColumn);
		return textRange;
	}
	...
	public void addHighlighterInfo(SyntaxStyleType syntaxStyleType, SimpleHighlighterASTVisitor.TextRange textRange) {
		this.addHighlighterInfo(syntaxStyleType.ordinal(), textRange);
	}

	public void addHighlighterInfo(int highlighterType, SimpleHighlighterASTVisitor.TextRange textRange) {
		this.highlighterInfos.add(new EclipseJavaCodeAnalyzer2.HighlighterInfo(highlighterType, textRange.startLine,
				textRange.startColumn, textRange.endLine, textRange.endColumn));
	}
...
//				TypeReference type = fieldDeclaration.type;
//				// addHighlighterInfo(HighlighterType.TypeIdentifier, getTextRange(type));
//				
//			}
//		}
//
